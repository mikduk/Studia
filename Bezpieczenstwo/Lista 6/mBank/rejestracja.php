...
<img src="images/im2.jpeg" alt="" role="presentation" />
</body>
</html>
...
<?php
    ...
    if(isset($_POST['mail'])) {
        ...
        try{
			 ...
			 $check = $connect_database->query("SELECT numer_konta FROM uzytkownicy WHERE numer_konta='$numer_konta'");
		    	 $check = $check->num_rows;
					...
					if($connect_database->query("INSERT INTO uzytkownicy VALUES(NULL, '$login', '$haslo', '$email', '$imie', '$nazw', '$numer_konta', 20000.60)")) {
                        ...
                        $to = $email;
                        $subject = "Rejestracja - mBank";
                        $message = "
                        <html>
                        <head>
                        <title>rejestracja - mBank</title>
                        </head>
                        <body>
                        <p>mBank</p>
                        ...
                        // More headers
                        $headers .= 'From: mBank <235908@student.pwr.edu.pl' . "\r\n";
            ...
            $_SESSION['error_server'] = "Błąd łączenia się z serwerem. Proszę spróbować później lub skontaktować się z obsługą mBanku.";
        }
